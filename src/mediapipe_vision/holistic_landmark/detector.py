...
class HolisticLandmarkDetector:
    ...
    def detect(
    ...
    ) -> Tuple[List[HolisticLandmarksResult], Optional[List[torch.Tensor]]]:
        ...
        for i in range(batch_size):
            ...
            holistic_results = self._detector_instance.process(cv2.cvtColor(np_image, cv2.COLOR_BGR2RGB))
    ...
    def _convert_to_landmark_result(self, holistic_results: Any) -> HolisticLandmarksResult:
        ...
        if holistic_results.pose_landmarks:
            pose_landmarks = [
                LandmarkPoint(index=idx, x=landmark.x, y=landmark.y, z=landmark.z, visibility=landmark.visibility)
        ...
        if holistic_results.left_hand_landmarks:
            left_hand_landmarks = [
                LandmarkPoint(index=idx, x=landmark.x, y=landmark.y, z=landmark.z)
