...
class LandmarkPositionBase:
    ...
    def extract_positions(
        self, landmarks: list, landmark_index: int, result_index: int = 0, use_world_coordinates: bool = False
                        ...
                        try:
                            ...
                            if isinstance(result_item, list):
                                ...
                                if use_world_coordinates and self.LANDMARKS_TYPE == "FACE_LANDMARKS":
                                    logger.warning(
                                        f"World coordinates requested but not available for FACE_LANDMARKS. Using image coordinates."
                                    )
                            ...
                            # Case 2: result_item is an object with .landmarks (HAND/POSE_LANDMARKS)
                                ...
                                if landmarks_attr is not None:
                                    use_world = (
                                        ...
                                        and self.LANDMARKS_TYPE != "FACE_LANDMARKS"
                                        and hasattr(result_item, "world_landmarks")
                                        and getattr(result_item, "world_landmarks", None)
                                    )
                                    if use_world:
                            ...
                            logger.error(
                                f"{self.__class__.__name__}: Unexpected error processing batch item {batch_idx}: {e}",
                                exc_info=True,
                            )
                            # Keep landmark_point as None, is_valid as False
                else:
                    logger.debug(
                        f"{self.__class__.__name__}: Result index {result_index} exceeds available detections for batch item {batch_idx} (has {len(item_list)})"
                    )
