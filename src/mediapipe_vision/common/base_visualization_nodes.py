...
class BaseLandmarkVisualizationNode:
    ...
    CATEGORY = "Realtime Nodes/MediaPipe Vision/Visualization"
    RETURN_TYPES = ("IMAGE",)
    ...
    def INPUT_TYPES(cls):
        ...
        if cls.SUPPORTS_VISIBILITY:
            inputs["optional"]["min_visibility"] = (
                "FLOAT",
                {
                    "default": 0.5,
                    "min": 0.0,
    ...
    def draw_visualizations(self, landmarks, image, **kwargs):
        ...
        if len(landmarks) != batch_size:
            logger.error(
                f"{self.__class__.__name__}: Batch size mismatch. Images: {batch_size}, Landmark sets: {len(landmarks)}. Cannot proceed reliably."
            )
            return (image,)
...
class BaseDetectionVisualizationNode:
    ...
    def get_text_dimensions(cls, text, font_scale=None, thickness=None):
        ...
        try:
            (text_width, text_height), baseline = cv2.getTextSize(text, cls.FONT, font_scale, thickness)
    ...
    def draw_text_with_background(
        ...
        # Draw text
        cv2.putText(image, text, (x + x_pad, y + text_height), self.FONT, font_scale, text_color_bgr, thickness)
    ...
    def process_batch(self, detections, image, drawing_function, **kwargs):
        ...
        if len(detections) != batch_size:
            logger.error(
                f"{self.__class__.__name__}: Batch size mismatch. Images: {batch_size}, Detection sets: {len(detections)}. Cannot proceed."
            )
            return (image,)
