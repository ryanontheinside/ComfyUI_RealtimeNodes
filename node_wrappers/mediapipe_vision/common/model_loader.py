...
class MediaPipeModelLoaderBaseNode:
    ...
    CATEGORY = "Realtime Nodes/MediaPipe Vision/Loaders"
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "model_variant": (task_variants, {"default": default_variant, "tooltip": tooltip}),
            }
        }
    ...
    def load_and_get_info(self, model_variant: str):  # Removed task_type argument
        ...
        if "model_variant" not in model_info:
            model_info["model_variant"] = model_variant

        logger.info(
            f"Successfully loaded model info: Path='{model_info.get('model_path')}', Task='{model_info.get('task_type')}', Variant='{model_info.get('model_variant')}'"
        )
